...
class TransformerPricePredictor(nn.Module):
    ...
    def __init__(self, input_features: int = 5, num_tech_features: int = 7, seq_length: int = 15,
                 d_model: int = 128, n_heads: int = 8, n_layers: int = 6, d_ff: int = 512, dropout: float = 0.1):
        super().__init__()
        ...
        # Model architecture
        self.input_projection = nn.Linear(input_features + num_tech_features, d_model)
        self.pos_encoding = PositionalEncoding(d_model, seq_length * 2)
...
class TransformerPriceModel(nn.Module):
    """Transformer model wrapper compatible with the framework."""
    def __init__(self, input_features: int = 5, num_tech_features: int = 7, seq_length: int = 15,
                 d_model: int = 128, n_heads: int = 8, n_layers: int = 6, dropout: float = 0.1,
                 learning_rate: float = 0.0001, batch_size: int = 32, epochs: int = 100, patience: int = 15):
        super().__init__()
        self.input_features = input_features
        self.num_tech_features = num_tech_features
        self.seq_length = seq_length
        ...
        self.model = TransformerPricePredictor(
            ...
            num_tech_features=num_tech_features,
        ...
        self.to(self.device)
        logger.info(f"Initialized Transformer model with device: {self.device}")
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray, X_val: Optional[np.ndarray] = None,
        ...
        self.model.input_center.data = torch.tensor(self.scaler_X.center_, dtype=torch.float32)
        self.model.input_scale.data = torch.tensor(self.scaler_X.scale_, dtype=torch.float32)
        self.model.price_center.data = torch.tensor(self.scaler_y.center_, dtype=torch.float32)
        self.model.price_scale.data = torch.tensor(self.scaler_y.scale_, dtype=torch.float32)

        X_tensor = torch.tensor(X_scaled, dtype=torch.float32).to(self.device)
        with torch.no_grad():
            tech_features = self.model.compute_technical_features(X_tensor)
            self.model.tech_center.data = tech_features.median(dim=1)[0].median(dim=0)[0]
            self.model.tech_scale.data = (tech_features.quantile(0.75, dim=1) - tech_features.quantile(0.25, dim=1)).median(dim=0)[0] + 1e-8
    ...
    def get_model_info() -> Dict[str, Any]:
        ...
        return {
            'name': 'transformer',
            'type': 'transformer',
            'description': 'Transformer-based deep neural network with self-attention and technical indicators for price prediction',
...
def create_model(**kwargs) -> 'TransformerPriceModel':
    """Factory function to create model instance."""
    return TransformerPriceModel(**kwargs)